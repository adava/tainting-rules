...
typedef enum s_op shift_op;

typedef void (*guest_memory_read_func)(uint64_t vaddr, uint8_t *buf, int len);
...
shadow_err SHD_and_or(shad_inq src, shad_inq *dst, guest_memory_read_func, logical_op op); // A custom rule that needs runtime operand values

shadow_err SHD_Shift_Rotation(shad_inq *dst, shad_inq src, shift_op op); //handles all these operations in one function since the workflow is the same

shadow_err SHD_copy_conservative(shad_inq src, shad_inq *dst);

//shadow_err SHD_set_eflags(shad_inq src, shad_inq *dst); use SHD_conservative with flags id
//shadow_err SHD_clear_eflags(shad_inq flgs); use SHD_clear
//shadow_err SHD_mul(shad_inq dst, shad_inq src1, shad_inq src2);  For now, use conservative.
